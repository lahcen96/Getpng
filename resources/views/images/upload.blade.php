...
$("#filePhoto").on('change', function(){
	...
	if(window.File && window.FileReader && window.FileList && window.Blob){
		if($(this).val()){ //check empty input filed
			...
			var allowed_file_size = {{$settings->file_size_allowed * 7024}};

			if(fsize>allowed_file_size){
				$('#filePhoto').val('');
				$('.popout').addClass('popout-error').html("{{trans('misc.max_size').': '.App\Helper::formatBytes($settings->file_size_allowed * 7024)}}").fadeIn(500).delay(5000).fadeOut();
				return false;
			}
